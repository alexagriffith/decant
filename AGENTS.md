...
A TypeScript/Bun single-app migration is in progress at the repo root. Until
cutover, keep the Rust/Phoenix implementation working and treat the TypeScript
tree as the parity target being built from golden fixtures generated by the
Rust implementation.
...
| `src/` | TypeScript single-app migration target: schema, model, cost, parsers, ingest, CLI/UI code as it ports over. |
| `test/golden/` | Synthetic Rust-generated JSON oracle for the TypeScript migration. Regenerate with `just ts-gen-goldens`. |
...
- TypeScript: Bun 1.3+ (`bun install --frozen-lockfile`).
...
just ts-check                                # TS gate: test + typecheck + Biome
just ts-gen-schema                           # regenerate src/schema.sql from Rust
just ts-gen-goldens                          # regenerate test/golden/ from Rust
...
- TypeScript: `bun test` **and** `bunx tsc --noEmit` **and** `bunx biome check .` all pass.
...
- **Never commit real session data or a personal archive DB.** `~/.claude` and `~/.codex` hold private transcripts. Keep committed fixtures synthetic; `test/golden/` is generated from `fixtures/` and should contain no machine-specific paths beyond the intentional `/Users/dev/proj` fixture path.
