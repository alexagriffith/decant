defmodule DecantWeb.Components.Icons do
  @moduledoc """
  Brand/model icons (single-color, inherit `currentColor`) plus dispatchers
  that pick the right mark for a model id or tool. Marks are the official
  single-color glyphs from Simple Icons (CC0).
  """
  ...
  def claude(assigns) do
    assigns = assign(assigns, :d, @claude)
  ...
  def openai(assigns) do
    assigns = assign(assigns, :d, @openai)
  ...
  def anthropic(assigns) do
    assigns = assign(assigns, :d, @anthropic)
  ...
  @doc "Which brand a model id belongs to: :claude | :openai | :other."
  def brand(nil), do: :other

  def brand(model) do
    m = String.downcase(model)

    cond do
      String.contains?(m, "claude") -> :claude
      String.contains?(m, "opus") -> :claude
      String.contains?(m, "sonnet") -> :claude
      String.contains?(m, "haiku") -> :claude
      String.contains?(m, "gpt") -> :openai
      String.contains?(m, "codex") -> :openai
      String.starts_with?(m, "o1") -> :openai
      String.starts_with?(m, "o3") -> :openai
      true -> :other
    end
  end
end
